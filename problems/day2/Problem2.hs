        ...
        Right parsedGames ->
          print $
            sum $
              map
                ( \(Game id extractions) ->
                    let computed = foldl' (\(Extraction redAcc greenAcc blueAcc) (Extraction redCur greenCur blueCur) -> Extraction (max redAcc redCur) (max greenAcc greenCur) (max blueAcc blueCur)) (Extraction 0 0 0) extractions
                     in redCount computed * greenCount computed * blueCount computed
                )
                parsedGames
