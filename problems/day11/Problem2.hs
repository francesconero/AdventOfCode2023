...
expand toExpand =
  let (out, _, _) =
        foldl'
          ( \(out, lastSeen, lastUsed) (k, v) ->
              let expansion = k - lastSeen - 1
                  newStart = lastUsed + expansion * 1000000 + 1
               in (out ++ [(newStart, v)], k, newStart)
          )
          ([], 0, 0)
          toExpand
   in out
      ...
      case runP parseAll 0 "" contents of
        Right parsed ->
          let expandedUniverse = rebuildGalaxies (expand $ groupByX parsed) (expand $ groupByY parsed)
           in --  in mapM_ print $ everyDistance expandedUniverse
              print $ sum $ map distance (everyDistance expandedUniverse)
