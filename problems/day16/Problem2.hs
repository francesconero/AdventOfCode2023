...
allEntrances :: Map.Map (Int, Int) Tile -> [((Int, Int), Ray)]
allEntrances m =
  [((0, y), E) | y <- [0 .. maxCol]]
    ++ [((x, 0), S) | x <- [0 .. maxRow]]
    ++ [((maxRow, y), W) | y <- [0 .. maxCol]]
    ++ [((x, maxCol), N) | x <- [0 .. maxRow]]
  where
    rows = Map.toList m
    maxRow = maximum (map (fst . fst) rows)
    maxCol = maximum (map (snd . fst) rows)

illuminate :: Map.Map (Int, Int) Tile -> ((Int, Int), Ray) -> Map.Map (Int, Int) Tile
illuminate m i = snd (propagateLight m [i])
  ...
  case args of
    [filePath] -> do
      ...
      case parse parseAll filePath contents of
        Right all -> do
          print $ maximum $ map (countEnergized . illuminate all) (allEntrances all)
